...
- Domain hierarchy: `User` → `TrainingSplit` (one active per user; tracks a current block) → `Workout` (a named training day) → `Exercise` (sets/reps/weight/video, ordered by `orderIndex`). A `TrainingLog` is a session started from a workout and holds `ExerciseLog`s.
